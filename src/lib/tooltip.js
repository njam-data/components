...
import { table as Table } from '@observablehq/inputs'
import { html } from 'htl'

// Based on https://observablehq.com/@fil/experimental-plot-tooltip-01

export function createTooltip (Plot) {
  return tooltip(Plot)
}
...
function defaultFormatter () {
  const formatter = objectFormatter();
  console.log('default????')
  return (div, value, { direction } = {}) => {
    console.log('defaultFormatter', value)
    if (typeof value === "object") return formatter(div, value, { direction });
...
function (div, obj, { direction } = {}) {
  console.log('objectFormatter', obj)
  // TODO: direction
...
function arrowTextFormatter(div, value, { direction } = {}) {
  console.log('arrowTextFormatter', value)
...
function ownerFigure(g) {
  console.log('ownerFigure')
  const svg = g.ownerSVGElement;
  return svg?.parentElement?.nodeName === "FIGURE" ? svg.parentElement : svg;
}

const callout = (container, { formatter, direction, text, x, y, transform, dx, dy } = {}) => {
